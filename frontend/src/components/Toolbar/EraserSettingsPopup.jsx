...
export function EraserSettingsPopup() {
  ...
  const { clearCurrentPage } = useNotebookStore();
  ...
  if (activePopup !== 'eraser') return null;

  return (
    <div
      ref={popupRef}
      className="absolute top-14 left-1/2 -translate-x-1/2 z-40 w-72 bg-[#1e2126]/95 backdrop-blur-xl border border-neutral-700/80 rounded-2xl shadow-floating p-4 flex flex-col gap-4 text-neutral-200 animate-in fade-in zoom-in-95 duration-150"
    >
      {/* 1. Eraser Mode */}
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          {[
            ...
            { id: 'medium', label: 'Medium', size: '24px' },
            { id: 'large', label: 'Large', size: '44px' },
            { id: 'xlarge', label: 'XL', size: '68px' }
          ].map((item) => (
            <button
              key={item.id}
